...
Python 문자열(string)을 작성할 때 작은 따옴표로 감싸듯이 정규표현식(Regular Expression) 역시 `\'google\'` 이런 식으로 작은 따옴표 안에 작성하면 된다. 단, 반복되는 패턴이 있는 부분은 `\'go{2,4}gle\'` (알파벳 o를 2~4번 반복하라는 뜻) 이런 식으로 문자열 뒤에 반복 회수를 지정해준다.
